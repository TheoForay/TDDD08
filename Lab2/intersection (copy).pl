...
inter1([], S2, []).
inter1([H|T], S2, [H|TI]) :- memberl(H,S2), \+memberl(H,T), inter1(T,S2,TI).
inter1([H|T], S2, TI) :- memberl(H,S2), memberl(H,T), inter1(T,S2,TI).
...
%inter(S1, S2, I) - Creates in I the intersection of the two non sets S1 and S2
inter([], S2, []).
inter([H|T], S2, [H|TI]) :- memberl(H,S2), \+memberl(H,T), inter(T,S2,TI).
inter([H|T], S2, TI) :- memberl(H,S2), memberl(H,T), inter(T,S2,TI).
