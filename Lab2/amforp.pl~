...
eval(num(X),L,F,X).
eval(+(A, B),L,F, Z) :- eval(A,L,F, A_R), eval(B,L,F, B_R),
			Z is A_R + B_R.
eval(*(A, B),L,F, Z) :- eval(A,L,F, A_R), eval(B,L,F, B_R),
			Z is A_R * B_R.
eval(-(A, B),L,F, Z) :- eval(A,L,F, A_R), eval(B,L,F, B_R),
...
eval(if(B,C,C1), L, F, Z) :- eval(B, L, F, _) -> eval(C,L,F,Z) ;
			     eval(C1,L,F,Z).

eval(while(tt,skip), L, F, L).
eval(while(B,C), L, F, Z) :- eval(B, L, F, _) -> eval(C,L,F,CR),
			     eval(while(B,C), CR, F, Z) ;
			     eval(while(tt, skip), L, F, Z).
