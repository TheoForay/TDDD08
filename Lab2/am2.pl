...
eval(greater(A, B), L, F, Z) :- eval(A,L,F, A_R), eval(B,L,F, B_R), A_R > B_R.
eval(less(A, B), L, F, Z) :- eval(A,L,F, A_R), eval(B,L,F, B_R), A_R =< B_R.
...
eval(while(true,skip), L, F, L).
eval(while(B,C), L, F, Z) :- eval(B, L, F, _) -> eval(C,L,F,CR), eval(while(B,C), CR, F, Z) ; eval(while(true, skip), L, F, Z).
